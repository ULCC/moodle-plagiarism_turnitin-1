...
class plagiarism_plugin_turnitin extends plagiarism_plugin {
                ...
                if ($plagiarismfile) {
                    ...
                    if ($plagiarismfile->statuscode == 'success' || ($plagiarismfile->statuscode == 'error' &&
                        ...
                        if (($istutor || (in_array($USER->id, $submissionusers) &&
                            ...
                            if ($cm->modname == 'coursework' && $finalised &&((!$hasgrade) || ($hasgrade && !$gradesreleased)) && !$istutor){
                                $useropenclass = '';
                            }
                        ...
                        if ($config->plagiarism_turnitin_usegrademark && ($istutor || ($linkarray["userid"] == $USER->id && $released)) && !empty($gradeitem)
                                && ($cm->modname != 'coursework' || ($cm->modname == 'coursework' && ($moduleobject->can_grade($submission, $coursework) || ($linkarray["userid"] == $USER->id && $released))))) {
    ...
    public function cron_update_scores() {
        ...
        foreach ($submissions as $tiisubmission) {
            ...
            if ($tiisubmission->modname == 'coursework'){ // for coursework use deadline
                $coursework = new \mod_coursework\models\coursework($tiisubmission->instance);
