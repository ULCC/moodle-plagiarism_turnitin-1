...
class sync_grades extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $sql = "SELECT ptc.id, ptc.cm, ptc.name, ptc.value, ptc.config_hash, m.name AS modtype,
                      COALESCE(a.duedate, q.timeclose, f.cutoffdate, w.submissionend, c.deadline) AS duedate
            ...
            LEFT JOIN {coursework} c ON (m.name = 'coursework' AND c.id = cm.instance)
                WHERE ptc.name = :configname
                  AND m.name IN ('assign', 'quiz', 'forum', 'workshop', 'coursework')";
